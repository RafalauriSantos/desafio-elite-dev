...
def run_e2e_test():
    ...
    with sync_playwright() as p:
        ...
        print("[STEP 1] Accessing Home Catalog (http://localhost:5173)...")
        ...
        organizer_btn = page.query_selector("button:has-text('Painel do Organizador')")
        if organizer_btn:
            organizer_btn.click()
            time.sleep(1)
        ...
        select_seats_btn = page.query_selector("button:has-text('Selecionar Assentos')")
        if select_seats_btn:
            select_seats_btn.click()
            time.sleep(1)
            page.screenshot(path="tests/screenshots/03_seat_map.png")
            print("  -> Seat Map rendered with curved stage screen.")
            ...
            if available_seat:
                ...
                reserve_btn = page.query_selector("button:has-text('Reservar & Ir para Pagamento')")
                if reserve_btn:
                    reserve_btn.click()
                    time.sleep(1)
                    page.screenshot(path="tests/screenshots/05_checkout_modal.png")
                    print("  -> Checkout Modal loaded with retention timer.")
                    
                    # Fill Customer Form & Confirm Payment
                    name_input = page.query_selector("input[placeholder*='Nome']")
                    if name_input:
                        name_input.fill("Rafa Tester")
                    
                    confirm_pay_btn = page.query_selector("button:has-text('Confirmar Pagamento Simulado')")
                    if confirm_pay_btn:
