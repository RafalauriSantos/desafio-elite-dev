...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="w-full sm:max-w-md bg-[#111113] rounded-t-2xl sm:rounded-2xl border border-zinc-800/80 shadow-2xl relative max-h-[85dvh] flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200">
        
        {/* Header */}
        <div className="shrink-0 p-5 pb-3 border-b border-zinc-800/40 relative">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 text-zinc-500 hover:text-white p-1.5 rounded-lg hover:bg-zinc-800 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <div className="flex-1 overflow-y-auto overscroll-contain p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-zinc-900/70 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-zinc-600 transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-zinc-900/70 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-sm text-white outline-none focus:border-zinc-600 transition-colors cursor-pointer"
              >
          ...
          {/* Action Footer (Always Pinned and Visible) */}
          <div className="shrink-0 p-4 sm:p-5 pt-3 pb-[calc(1.5rem+env(safe-area-inset-bottom))] bg-[#111113] border-t border-zinc-800/80 flex gap-2.5 shadow-2xl">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 rounded-lg border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800/60 text-sm font-medium transition-colors"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-[2] py-3 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-semibold text-sm transition-colors disabled:opacity-40 flex items-center justify-center shadow-lg shadow-emerald-950/20"
            >
              {loading ? (
